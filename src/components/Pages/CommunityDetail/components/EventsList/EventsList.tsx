...
import { EmptyState, EmptyStateText, EventsGrid, EventsSection, LoadMoreSentinel, SectionTitle } from './EventsList.styled'
  ...
  return (
    <EventsSection>
      ...
      {events.length === 0 ? (
        <EmptyState>
          <EmptyEventsIcon />
          <EmptyStateText color="textPrimary">{t('events.no_upcoming_events')}</EmptyStateText>
        </EmptyState>
      ) : (
        <EventsGrid>
          {events.map(event => (
            <EventItem key={event.id} event={event} attendeePictures={attendeePictures} />
          ))}
          {hasMore && (
